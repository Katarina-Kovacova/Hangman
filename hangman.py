...
def update_hidden_positions(keep_hidden, remove_guessed_positions):
	# print(remove_guessed_positions)
	# print(keep_hidden)
...
def update_guessed_leters (guess, guessed_letters): #function will update the list with already guessed letters and return updated list
	...
	print(list(guessed_letters_set))
	...
	while not game_over:
		...
		if guess in guessed_letters:
			...
			if guess in ran_word:
				...
				# bad_guesses = bad_guesses + 1 (same as line 115)
				bad_guesses += 1
				print(get_body_parts(bad_guesses))
...
quit the game when they got all body parts printed 
stop printnig letter position when guessed correctly
